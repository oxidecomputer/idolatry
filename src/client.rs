...
pub fn generate_client_stub(
    ...
    // Silence a few Clippy complaints about the autogenerated code
    writeln!(out, "#[allow(clippy::let_unit_value,")?;
    writeln!(out, "        clippy::collapsible_else_if,")?;
    writeln!(out, "        clippy::needless_return,")?;
    writeln!(out, "        clippy::unused_unit)]")?;
    ...
    for (idx, (name, op)) in iface.ops.iter().enumerate() {
        ...
        // Open retry loop for idempotent operations.
        if op.idempotent {
            writeln!(out, "        loop {{")?;
        }
        ...
        if op.idempotent {
            writeln!(out, "        if let Some(g) = userlib::extract_new_generation(rc) {{")?;
            ...
            syntax::Reply::Simple(t) => {
                ...
                match &t.recv {
                    syntax::RecvStrategy::FromBytes => {
                        match t.ty.0.as_str() {
                            "bool" => writeln!(out, "        return v != 0;"),
                            _ => writeln!(out, "        return v;"),
                        }?;
                    }
                    syntax::RecvStrategy::From(_, None) => {
                        writeln!(out, "        return v.into();")?;
                    }
                    syntax::RecvStrategy::From(_, Some(f)) => {
                        writeln!(out, "        return {}(v);", f)?;
                    }
                    syntax::RecvStrategy::FromPrimitive(p) => {
                        writeln!(out, "        return <{} as userlib::FromPrimitive>::from_{}(v).unwrap_lite();", t.ty.0, p.0)?;
                    }
                }
            }
            syntax::Reply::Result { ok, err } => {
                ...
                match &ok.recv {
                    syntax::RecvStrategy::FromBytes => {
                        match ok.ty.0.as_str() {
                            "bool" => {
                                writeln!(out, "            return Ok(v != 0);")
                            }
                            _ => writeln!(out, "            return Ok(v);"),
                        }?;
                    }
                    syntax::RecvStrategy::From(_, None) => {
                        writeln!(out, "            return Ok(v.into());")?;
                    }
                    syntax::RecvStrategy::From(_, Some(f)) => {
                        writeln!(out, "            return Ok({}(v));", f)?;
                    }
                    syntax::RecvStrategy::FromPrimitive(p) => {
                        writeln!(out, "            return Ok(<{} as userlib::FromPrimitive>::from_{}(v).unwrap_lite());", ok.ty.0, p.0)?;
                    }
                }
                writeln!(out, "        }} else {{")?;
                match err {
        ...
        // Close retry loop for idempotent operations.
        if op.idempotent {
            writeln!(out, "        }}")?;
        }
